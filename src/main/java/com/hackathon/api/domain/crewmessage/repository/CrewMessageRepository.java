...
public interface CrewMessageRepository extends JpaRepository<CrewMessage, UUID> {
    ...
    List<CrewMessage> findByCrew_IdOrderByCreatedAtAsc(UUID crewId);

    List<CrewMessage> findByCrew_IdAndCreatedAtAfterOrderByCreatedAtAsc(UUID crewId, LocalDateTime after);
}
